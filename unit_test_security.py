...
class Security:
    ...
    def recompute_indicators(self):
        ...
        if self.is_currency == False:
            ...
            print("VPIN: %s" % vpin_results['VPIN'].values[-1])
        ...
        print(self.ticker,self.is_currency,self.indicators)

    """ ------- Security State -----------"""
    def get_midprice(self):
        print('bid ask')
        print(self.best_bid_ask['midprice'])
    ...
    def compute_historical_vpin(self, all_trades,BAR_SIZE='1min',N_BUCKET_SIZE=10,SAMPLE_LENGTH=12):
        ...
        return {"VPIN": vpin_df, "pct_changes_df": pct_changes_df, "std_price_changes": std_price_changes, 'bucket_duration':pd.Series(diffs), 'bucket_mid_prices':pd.Series(bucket_mid_prices)}
...
def main():
    sec = Security('RITC', API(API_CONFIG, DB_PATH, SQL_CONFIG, use_websocket=False))
